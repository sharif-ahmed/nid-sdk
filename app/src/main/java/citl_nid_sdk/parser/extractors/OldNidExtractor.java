package citl_nid_sdk.parser.extractors;
...
import citl_nid_sdk.NidDocumentParser;
...
/**
 * Extractor for Old NID Card (Legacy).
 */
public class OldNidExtractor implements NidExtractor {

    @Override
    public NidData extract(List<String> lines, String rawText) {
        NidData data = new NidData();
        data.setCardType("OLD");

        /*for (String line : lines) {
            if (CorrectionLayer.containsFuzzy(line, "নাম")) {
                data.setNameBn(CorrectionLayer.cleanField(line));
            } else if (line.startsWith("Name:") || line.startsWith("Name ")) {
                data.setNameEn(line.replace("Name:", "").replace("Name", "").trim());
            } else if (CorrectionLayer.containsFuzzy(line, "পিতা")) {
                data.setFatherName(CorrectionLayer.cleanField(line, "father"));
            } else if (CorrectionLayer.containsFuzzy(line, "মাতা")) {
                data.setMotherName(CorrectionLayer.cleanField(line, "mother"));
            }
        }*/
        NidDocumentParser.NidData nidData = parse(rawText);
        data.setNameEn(nidData.name_en);
        data.setNameBn(nidData.name_bn);
        data.setDateOfBirth(nidData.date_of_birth);
        data.setFatherName(nidData.father_name);
        data.setMotherName(nidData.mother_name);
        data.setNidNumber(nidData.nid_number);
        return data;
    }
    /**
     * Parse raw OCR text into structured NidData.
     */
    public static NidDocumentParser.NidData parse(String rawText) {
        if (rawText == null || rawText.isEmpty()) return new NidDocumentParser.NidData();
        ...
        NidDocumentParser.NidData data = new NidDocumentParser.NidData();
    ...
    private static void extractNamesAndParents(List<String> lines, NidDocumentParser.NidData data) {
        int engNameIndex = -1;
