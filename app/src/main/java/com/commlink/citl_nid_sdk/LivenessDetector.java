...
public class LivenessDetector {
    ...
    public LivenessState processFace(@NonNull List<Face> faces) {
        ...
        switch (currentAction) {
            case BLINK:
                targetProgress = detectBlink(face);
                // Smooth interpolation - only update current action progress
                state.blinkProgress = interpolateProgress(state.blinkProgress, targetProgress);
                if (state.blinkProgress >= 0.9f && !state.blinkDone) {
    ...
    private float detectBlink(@NonNull Face face) {
        ...
        if (leftEyeProb == null || rightEyeProb == null) {
            return state.blinkProgress; // Keep current progress
        }

        float avgProb = (leftEyeProb + rightEyeProb) / 2.0f;

        // Progress increases as eyes close (probability decreases)
        // Simple mapping: eyes fully open (1.0) -> 0, fully closed (0.0) -> 1
        float progress = 1.0f - avgProb;
